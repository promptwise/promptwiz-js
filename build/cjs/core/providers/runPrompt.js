  ...
  return __async(this, null, function* () {
    ...
    while (++retries <= max_retries) {
      try {
        ...
        const { outputs, original, usage } = yield promptRunner(__spreadProps(__spreadValues(__spreadValues({}, config), current), {
          access_token: TOKEN
        }));
        ...
        if (strategy && strategy.after >= max_retries) {
          ...
          if (!parameters) {
            parameters = _next.provider === config.provider ? config.parameters : _next.provider === current.provider ? current.parameters : config.parameters ? (0, import_getProvider.getProvider)(_next.provider).parametersFromProvider(
              config.provider,
              config.parameters
            ) : void 0;
          }
